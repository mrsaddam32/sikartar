...
<div class="content-wrapper">
    ...
    <section class="content">
        <form action="{{ route('admin.activity.update', ['activities_id' => $activity->activity_id]) }}" method="POST">
            @csrf
            @method('PUT')
                            ...
                            <div class="form-group">
                                ...
                                <select id="inputStatus" name="responsible_person" class="form-control custom-select">
                                    ...
                                    @foreach ( $users as $user )
                                        <option value="{{ $user->name }}" {{ $user->name == $activity->responsible_person ? 'selected' : '' }}>{{ $user->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="inputStatus">Activity Status</label>
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <button class="btn btn-info" type="submit">Edit Activity</button>
                </div>
            </div>
        </form>
    </section>
